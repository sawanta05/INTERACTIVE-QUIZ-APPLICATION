...
The program was developed using Visual Studio Code (VS Code), which is a popular and powerful code editor. VS Code provides features such as syntax highlighting, code formatting, and live preview, making it easier to write and debug the code efficiently. The application runs on a web-based platform, meaning it can be accessed through any modern web browser such as Google Chrome, Microsoft Edge, or Mozilla Firefox. It is compatible with multiple operating systems including Windows, Linux, and macOS.

One of the key features of the application is its interactive quiz mechanism. When the quiz starts, questions are displayed one by one along with four options. The user selects an answer, and immediate feedback is provided indicating whether the answer is correct or incorrect. A timer counts down for each question, and if the user fails to answer within the time limit, the question is marked incorrect. The application also includes a scoring system that awards points based on correct answers and remaining time, encouraging quick and accurate responses.

At the end of the quiz, a result screen is displayed showing the user’s performance, including total score percentage, number of correct and incorrect answers, and average time taken per question. Additionally, the application provides a review section where users can see all the questions along with their selected answers and the correct answers, helping them learn from their mistakes.
